...
public class ProcurementReportsAction extends DispatchAction {
	...
	private void setWhere(ProcurementReportsForm procurementReportsForm,HttpServletRequest request){
		...
		//KD WORK
		String idrb_fromForm = procurementReportsForm.getiDRB();
		System.out.println("IRDB FOM DATA IS: " + idrb_fromForm);
		//KD WORK
		...
		String project="1=1 ";
		String prog="1=1 "; 
		String scheme_type="1=1 ";
		String review="1=1 ";
		String village_category = idrb_fromForm.trim();
				...
				if(schemeType.equals("all")){
					...
					scheme_types = "all";
						...
						if(schemeType.equals("ws")){
							...
							innerWhere=innerWhere+"  and ws_sw in ('imp','wm')";
							System.out.println("inner where==="+innerWhere);
						}
				}
		...
		else
		{
			if(MisUtility.ifEmpty(schemeType)){
				if(schemeType.equals("all")){
					...
					if(schemeType.equals("imp")){
					where=where+" and scheme_source in ('imp','wm')";
					}
					else{
		...
		if(selectReport.equals("procrpt001_1")||selectReport.equals("procrpt001_4")||selectReport.equals("procrpt001_13")||selectReport.equals("procrpt001_3")){
			System.out.println("select report is ===="+selectReport);
			if(selectProgram.equals("S")){
				System.out.println("inside s === selected");
				System.out.println("program id  before if ===="+programId);
			     if(programId.equals("'PROG30'")){
					System.out.println("program id  is of 2 project ===="+programId);
					project="PRWSS-II";
				}
				else{
		...
		if(selectReport.equals("procrpt001_2")){
			System.out.println("post Prior Status===="+postPriorStatus);
			System.out.println("Selected IDB is ===  " + idrb_fromForm);
			...
			if(schemeType.equals("ws")){
				...
				scheme_type = "Water Supply";
			}else if(schemeType.equals("sw")){
				scheme_type = "Sewerage";
			}else if(schemeType.equals("imp")){
				scheme_type = "Performance Improvement";
			}
			else if(schemeType.equals("A")||schemeType.equals("")){
				scheme_type="All";
			}
			
		
			if(postPriorStatus.equals("PRIOR-REVIEW")||postPriorStatus=="PRIOR-REVIEW"){
				System.out.println("prior - review ==="+postPriorStatus);
				review="Prior-Review";
			}
			else if(postPriorStatus.equals("POST-REVIEW")||postPriorStatus=="POST-REVIEW"){
				System.out.println("post - review ==="+postPriorStatus);
				review="Post-Review";
			}
			else{
				review="All";
		...
		if(selectProgram.equals("S")){
			...
			if(selectReport.equals("procrpt001_1")||selectReport.equals("procrpt001_4")||selectReport.equals("procrpt001_3")){
				System.out.println("2 selectProgram: "+selectProgram);
				prog_id="  prog_id="+programId;
			}
			if(selectReport.equals("procrpt001_13")){
				System.out.println("3 selectProgram: "+selectProgram);
				prog="  prog="+programId;
			}
			/*else{
				programId= " 1=1 ";
		...
		if(selectReport.equals("procrpt001_2")){
			...
			if(programId.equals("'PROG30'")){
				...
				if(idrb_fromForm.equals("1A")
						||idrb_fromForm.equals("1A (WL)")
						||idrb_fromForm.equals("1B")
						||idrb_fromForm.equals("3A")
						||idrb_fromForm.equals("3B1")
						||idrb_fromForm.equals("3B2")
						||idrb_fromForm.equals("2A")
						||idrb_fromForm.equals("2B")){
					where+=" and village_category='"+village_category+"'";
					System.out.println(where);
				}else if (idrb_fromForm.equals("ALL")) {
					where = where+" and village_category in('1A (WL)','1A','1B','3A','3B1','3B2','2A','2B')";
					System.out.println(where);
				}
				else{
					where+=" and village_category='"+""+"'";
					System.out.println(where);
				}
		...
		System.out.println("This is the program Id that is going"+programId);
		...
		System.out.println("village_category====="+village_category);
